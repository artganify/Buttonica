...
namespace Buttonica.Core.Framework.Core.Types
{

	/// <summary>
	///		Represents a rectangle using floating point values
	/// </summary>
	public struct RectangleF
	{
		...
		///		Returns whether the specified <see cref="Vector2"/> lies within the bounds of this <see cref="RectangleF"/>.
		/// </summary>
		public void Contains(ref Vector2 value, out bool result) => result = (X <= value.X) && (value.X < X + Width) && (Y <= value.Y) && (value.Y < Y + Height);
		...
		public void Contains(ref RectangleF value, out bool result) => result = (X <= value.X) && 
			(value.X + value.Width <= X + Width) && (Y <= value.Y) && (value.Y + value.Height <= Y + Height);
